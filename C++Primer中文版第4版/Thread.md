 ...
 * 线程是一个程序的一系列执行。Java虚拟机允许应用程序同时运行多个线程的执行。
 * <p>
 ...
 * 每个线程都有一个优先级。高优先级的线程优先于低优先级的线程执行。每个程序都有可能被标记为守护线程也有可能不会被标记。当某个线程中运行的代码创建一个新线程对象，那么新线程初步设置的优先级等于创建线程的优先级，而且如果创建线程是一个守护进程，那么新线程也是一个守护线程。
 * <p>
 ...
 *     method.
 * </ul>
 * 当一个Java虚拟机启动的时候，通常会有一个单独的非守护线程，通常调用一些指定类的取名为main的方法。Java虚拟机会一直执行线程知道一下其中一个方法：Runtime类的exit方法被调用，并且安全管理器运行退出操作的发生。所有非守护线程都已经死亡，要么通过返回调用run方法，或者通过抛出异常传播以外的运行方法。
 * <p>
